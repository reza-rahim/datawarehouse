...
# === Root CA Certificate ===
openssl req -x509 -new -nodes -key ca/rootCA.key -sha256 -days $DAYS_VALID_CA -out ca/rootCA.pem \
...
openssl x509 -req -in server/server.csr -CA ca/rootCA.pem -CAkey ca/rootCA.key -CAcreateserial \
...
echo "Root CA Certificate: $(pwd)/ca/rootCA.pem"
