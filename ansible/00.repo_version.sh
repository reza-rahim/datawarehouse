export ANSIBLE_CONFIG=./ansible.cfg
#
export ZOOKEEPER=3.8.4

### library version
export HADOOP_VERSION=3.3.4
#hadoop_aws
export HADOOP_AWS=3.3.4
#aws_java_sdk_bundle
export AWS_JAVA_SDK_BUNDLE=1.12.262

#####
export AWS_BUNDLE=2.31.8
#
export SPARK_VERSION=3.5.6
#iceberge
export ICEBERG_SPARK_RUNTIME_MAJOR=3.5_2.13
export ICEBERG_SPARK_RUNTIME_MINOR=1.8.0

#awssdk_bundle
export AWSSDK_BUNDLE=2.20.8

#url-connection-client
export URL_CONNECTION_CLIENT=2.20.38

export HIVE_METASTOR=3.1.3
export MINIO=20250408154124.0.0
export POSTGRESQL_CONNECTOR=42.7.5

export TRINO=475
export TRINO_CLI=475

export KEYCLOAK=26.2.0

export PROMETHEUS=3.2.1
export NODE_EXPORTER=1.9.1
export GRAFANA=11.6.0


export JUPYTERHUB=5.1
export JUPYTERLAB=4.1
export NOTEBOOK=v7.2.1
export OAUTHENTICATOR=17.3.0
export SUDOSPAWNER=0.5.2
export PSYCOPG2_BINARY=2.9.10
export PLOTLY=6.0.1
export SEABORN=0.13.2
export MATPLOTLIB=3.5.3
export NUMPY=2.2.5
export PANDAS=2.2.3
export SCIKIT_LEARN=1.6.1
export SCIKIT_IMAGE=0.25.2
export STATSMODELS=0.14.4
export S3CMD=2.4.0
export SQLALCHEMY=2.0.40
export IPYTHON_SQL=0.5.0
export SPARKSQL_MAGIC=0.0.3
export TORCH=2.5.0
export SQLALCHEMY_TRINO=0.5.0

export APACHE_AIRFLOW=2.10.4

export OPENSEARCH=3.0.0
export OPENSEARCH_DASHBOARD=3.1.0

###########################
